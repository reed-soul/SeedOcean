...
烘焙纯 CPU：`bakeRiverFlow`（中心线切线）、`bakeShoreRing`（湖盘边缘）、`bakeShoreChannel`（河岸）、`bakeCoastalSurf`（深度破碎带 + 向岸 rush）。湖/河/海岸自动启用；ocean/pool 默认关闭（demo 仍分配空白图供画笔）。⚠️ 湖岸泡沫必须按 **水面网格边缘** 烘焙，不能按地形 `h≈0`——盆地在盘下约 −6 m，水位线交叉测试会把整个湖床刷白。海岸则相反：`terrain.beach` **会**穿过 y=0，所以用高度/深度烘焙。

**画笔（11d）**：`FlowMap.paint(mode)` + `toJSON`/`fromJSON`（base64 pixels）。Demo 用 Shift+拖拽；`SeedOcean.exportPreset()` 把 pixels 嵌进 `seedocean-preset/1`。
