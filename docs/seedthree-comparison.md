...
8. ✅ Coastal Surf (`waterType: 'coast'`) — 海滩坡面 + 深度破碎带 + 向岸 rush。

### 🟢 P2 — 后续

9. Demo 对象解耦：`_buildDemoObjects()` 写死 boat/buoy/crates → 改成 `demoObjects: () => [...]` 工厂或 preset 字段。
10. 注释纪律成文：把已有好注释（clipmap seam、flowOffset、celBands）的写作风格上升为项目规范。
11. ✅ Shoreline editor (Phase 11d) — Shift+drag brush + `exportPreset` 嵌入 `flowmap.pixels`。
