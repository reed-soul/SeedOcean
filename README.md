...
> **Status: `v0.7.1-alpha`.** Fourteen of nineteen presets above — realistic day/night/tropical/polar/bioluminescent, cel-shaded **Cartoon** and **Ink Wash** stylized modes, and three bounded water forms: a rectangular **Pool**, a circular **Mountain Lake** in a procedural fBm terrain basin, and a meandering **River** with directional flow (cascade-UV scroll + current force on buoyant bodies). FFT ocean with persistent/advected foam, a 256² quality mode, TypeScript types, a `<water-canvas>` web component, atmospheric spray + rain, a procedural star field, and a WebGL2/Gerstner fallback that keeps the API identical when WebGPU is unavailable.
...
const ocean = await SeedOcean.create({
  ...
  scene,
  camera,
  preset: 'coastal',   // any of the 17 presets
...
| 9 ✅ | v0.7.1 — River preset: directional flow (cascade-UV scroll shader) + Catmull-Rom ribbon mesh + buoyancy current force *(current)* |
| 10 🔜 | Shoreline editor / flowmap painter |
